...
export class LocaleParser implements ILocaleParser {
  ...
  private async getSpreadProperties(
    path: string,
    prop: SpreadElement,
    astbody: Statement[]
  ) {
  ...
  private async parseByIdentifier(
    path: string,
    identifier: string,
    astbody: Statement[]
  ) {
    const found = astbody.find((a) => {
      return (
        ...
        a.specifiers.some(
          (s) => isImportDefaultSpecifier(s) && s.local.name === identifier
        )
      );
    });
    ...
    if (
      !found ||
      !isImportDeclaration(found) ||
      !isStringLiteral(found.source)
    ) {
      return [];
    }
