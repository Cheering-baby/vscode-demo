...
import {
  IUmircParser,
  UmircParserToken,
} from "../../services/parser/umircParser";
...
export class UmircDecoration implements Disposable {
  ...
  constructor() {
    this.umircParser = Container.get(UmircParserToken);
    this.triggerUpdateDecorations();
    this.disposables.push(
      window.onDidChangeActiveTextEditor(
        this.triggerUpdateDecorations.bind(this)
      )
    );
    this.disposables.push(
      workspace.onDidChangeTextDocument(() => this.triggerUpdateDecorations())
    );
  }
  ...
  private validUmirc(editor: TextEditor | undefined): editor is TextEditor {
    ...
    if (basename(path).includes(".umirc")) {
      return true;
    }
    return (
      path.endsWith("config/config.js") || path.endsWith("config/config.ts")
    );
  }
}
